...
- **meetingStartDetection.patterns**: `string[]` - Case-insensitive regular expressions for the DOM fallback path (default: `["meeting started", "started the meeting"]`; English-only — override for other Microsoft 365 UI languages). Primary detection uses the web app's internal command stream and ignores these.
