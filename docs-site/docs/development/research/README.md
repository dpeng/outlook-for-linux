...
This directory contains research, analysis, and strategic insights generated during the development and improvement of Outlook for Linux.
...
- **[Documentation, Contributing, and Config UX](documentation-and-config-ux-research.md)** — `app/config/options.js` as the single source of truth feeding generated docs, an in-app settings UI, and startup validation ([#2597](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2597))
  - **Phases 0–2 shipped (v2.12.0)**: drift fixes ([PR #2602](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2602)), generated config reference + `config-schema.json` with CI drift guard ([PR #2604](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2604)), interactive config explorer in the docs site ([PR #2606](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2606))
...
- **[Join Meeting Window Takeover Research](join-meeting-window-takeover-research.md)** — "Join Meeting" replacing the whole window ([#2322](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2322))
...
- **[MQTT Extended Status Investigation](mqtt-extended-status-investigation.md)** - Extended MQTT status publishing
  ...
  - **Phase 2 Microphone Shipped**: [PR #2497](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2497) merged; publishes speaking/silent/muted/off to `{topicPrefix}/microphone`
  - **Phase 2 Camera Shipped**: [PR #2582](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2582) merged; renderer emits `camera-state-changed` via video track monitoring in speakingIndicator
  - **Home Assistant Auto-Discovery Shipped**: [PR #2464](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2464) and [PR #2571](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2571) merged; sensors, binary_sensors, and buttons auto-created in HA
...
- **[Smartcard / NSS PIN Dialog](smartcard-nss-pin-dialog-research.md)** — PIN dialog for password-protected PKCS#11 providers (smartcards) via Electron 33's `app.setClientCertRequestPasswordHandler` ([#2639](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2639))
  ...
  - UI feasibility analyzed: standalone always-on-top window styled like existing dialogs; in-page injection ruled out (security + handshake timing). Overlaps with the FIDO2 touch prompt ([#2631](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2631)) — whichever lands first builds a shared `app/_shared/` secure-prompt helper (no duplicated secret-input dialog)

- **[Custom Stickers — External Sources](custom-stickers-online-import-research.md)** — follow-up to the v1 ship ([#2476](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2476), PR [#2550](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2550))
...
- **[FIDO2 Touch Prompt UI](fido2-touch-prompt-research.md)** — surface a "touch your security key now" prompt during the user-presence wait ([#2631](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2631))
...
- **[Custom Notification System Research](custom-notification-system-research.md)** - Alternative notification modal system
  - **Initial version shipped** in v2.6.16: toast notifications with auto-dismiss and click-to-focus
...
|---------|---------|-----------|
| MQTT Microphone State | v2.10.0 | Speaking-indicator driven microphone state (speaking/silent/muted/off) published to MQTT. See [PR #2497](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2497) |
| Notification Sound Player (inline replacement for `node-sound`) | v2.7.10 | Phase 1 of the notification-sound research shipped — `paplay`/`pw-play`/`aplay`/`afplay` detection in `app/audio/player.js`. See [PR #2306](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2306) |
...
| Electron 41 Upgrade | v2.8.0 | Repo skipped Electron 40 entirely and jumped 39.8.2 → 41.x via dependabot [PR #2347](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2347), with follow-up bumps to 41.5.0; the Electron 40 migration research is therefore obsolete |
| Issue-PR Release Linking | v2.7.11 | GraphQL `closingIssuesReferences` query; `closes:` metadata in changelog files. See [PR #2317](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2317) |
...
| Speaking Indicator | v2.7.11 | WebRTC `getStats()` for three-state mute/speaking detection. See [PR #2299](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2299) |
...
| Screen Lock Media Privacy | --- | Closed ([#2106](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2106)); no user interest; work preserved in branch |
| Tray Icon Logout Indicator | --- | Archived ([#1987](https://github.com/IsmaelMartinez/outlook-for-linux/issues/1987)); user not responding; work preserved in branch |
| External Browser Authentication | --- | Not feasible; Teams manages OAuth internally ([#2017](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2017)) |
| GNOME Search Provider | --- | Not recommended; latency too high for acceptable UX ([#2075](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2075)) |
