...
- ⚠️ Per-profile state audit required for today's module-level singletons (enumerated under "Shared-state audit" in the Migration Plan; Phase 1 addresses login try-state and the per-session screen-share picker rebinding, plus one small mechanical refactor in `CustomBackground` — the screen-preview partition entry was rendered moot by the #2534 preview rework)
...
- Files to be touched:
  ...
  - `app/mainAppWindow/index.js` — screen-preview partition: no change required (the #2534 preview rework removed the session; see Shared-state audit)
  - `app/customBackground/index.js` — move `customBGServiceUrl` from module scope to a private instance field (landed); per-profile `CustomBackground` instantiation is deferred to the per-view work, since instantiating it more than once would double-register the `get-custom-bg-list` IPC handler and overlap webRequest handlers
