...
- **Media and Calls**: camera and mic handling, speaking indicator, call drops on multi-interface systems. Live: [media](https://github.com/IsmaelMartinez/outlook-for-linux/issues?q=is%3Aissue+is%3Aopen+label%3Amedia), [screen-sharing](https://github.com/IsmaelMartinez/outlook-for-linux/issues?q=is%3Aissue+is%3Aopen+label%3Ascreen-sharing). In review: suppress automatic audio/video device switching (`media.preventDeviceSwitching`, [PR #2652](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2652)).
- **Downloads**: download-feedback manager shipped --- taskbar/KDE JobView progress and completion notifications ([#2512](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2512)). In review: save options `download.saveDirectory` / `download.alwaysAskWhereToSave` / `download.openWhenDone` plus a policy-block notification for M365/SharePoint-interrupted downloads ([PR #2652](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2652)), and context-menu attachment download ("Download Attachment & Copy to Clipboard", "Save Open Document to File") through the authenticated session ([PR #2654](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2654)). All three split out of [#2645](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2645).
- **macOS Desktop Integration**: In review: presence-status overlay on the Dock icon (`media.showStatusOnDockIcon`) and opt-out Apple-Silicon rendering/performance switches (`media.macPerformanceMode`, larger V8 heap gated to arm64) ([PR #2653](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2653)).
...
- **MQTT Integration**: presence, media state, screen-share topics shipped. Home Assistant auto-discovery shipped ([PR #2464](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2464), [PR #2571](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2571)). Microphone state shipped ([PR #2497](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2497)). Incoming-call topic shipped ([PR #2572](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2572)). Camera topic shipped ([PR #2582](https://github.com/IsmaelMartinez/outlook-for-linux/pull/2582)). Live: [mqtt](https://github.com/IsmaelMartinez/outlook-for-linux/issues?q=is%3Aissue+is%3Aopen+label%3Amqtt).
...
- **Config Schema as Single Source of Truth** ([#2597](https://github.com/IsmaelMartinez/outlook-for-linux/issues/2597)): phases 0-2 shipped in v2.12.0 (drift fixes, generated reference + `config-schema.json` with CI drift guard, docs config explorer); 3a (`applyMode` + nested-field metadata, hard-failing generator lint) and 4 (warn-only startup validation) landed; 3b (in-app settings window) is the remaining phase ([research](../research/documentation-and-config-ux-research.md)). The settings window is the deliverable, and its restart-only initial version can ship on its own:
  - Build the restart-only settings initial version, reusing the `legacyConfigStore` override-on-boot layering and allowlisting writes to schema keys with type/choice plus deep prototype-pollution validation; defer the live-apply subset and the object-typed forms.
