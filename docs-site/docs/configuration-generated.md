...
| `screenSharing` | `object` | `{"enabled":false,"thumbnail":{"enabled":false,"alwaysOnTop":true},"lockInhibitionMethod":"Electron"}` | Screen sharing configuration. Disabled by default for Outlook for Linux. thumbnail: controls the preview window shown during active sharing. lockInhibitionMethod: screen lock inhibition method (Electron/WakeLockSentinel). | `restart` |
| `appIcon` | `string` | `""` | Outlook app icon to show in the tray | `restart` |
...
| `appTitle` | `string` | `"Outlook"` | A text to be suffixed with page title | `restart` |
...
| `idleDetection` | `object` | `{"forceState":false,"stateFile":"/tmp/outlook-for-linux-idle-state-$USER"}` | Idle detection configuration. forceState: enables state file-based idle control (workaround for Wayland/Hyprland). stateFile: path to state file with $USER expansion support. | `restart` |
...
| `followSystemTheme` | `boolean` | `true` | Follow the operating-system dark/light theme preference. Default is true; set false to keep Outlook's own theme regardless of OS changes. | `restart` |
...
| `meetupJoinRegEx` | `string` | `"$^"` | Regex for meeting links. Disabled by default for Outlook for Linux. | `restart` |
...
| `onNewWindowOpenMeetupJoinUrlInApp` | `boolean` | `false` | Open meetupJoinRegEx URLs in the app instead of the default browser. Disabled by default for Outlook for Linux. | `restart` |
| `partition` | `string` | `"persist:outlook-for-linux"` | BrowserWindow webpreferences partition | `restart` |
...
| `url` | `string` | `"https://outlook.office.com/mail/"` | Microsoft Outlook URL | `restart` |
| `useMutationTitleLogic` | `boolean` | `true` | Use MutationObserver to update counter from title | `restart` |
| `outlookUnreadCountObserver` | `object` | `{"enabled":true}` | Outlook-specific unread count observer. Publishes Inbox unread counts through the existing unread-count event when Outlook Web does not expose the count in the page title. | `restart` |
| `outlookNewMailNotifier` | `object` | `{"enabled":true}` | Outlook-specific new mail notifier. Shows native notifications with the sender and subject by observing Outlook Web's unread mail rows. | `restart` |
...
| `mqtt` | `object` | `{"enabled":false,"brokerUrl":"","username":"","password":"","clientId":"outlook-for-linux","topicPrefix":"outlook","statusTopic":"status","commandTopic":"","statusCheckInterval":10000,"homeAssistant":{"enabled":false,"discoveryPrefix":"homeassistant","deviceName":"Outlook for Linux"},"mediaTopics":{"inCall":"in-call","incomingCall":"incoming-call","camera":"camera","microphone":"microphone","microphoneControl":"microphone/control","screenSharing":"screen-sharing"}}` | MQTT configuration for publishing web app status updates and receiving action commands | `restart` |
...
|-------|------|---------|-------------|
| `screenSharing.enabled` | `boolean` | `false` | Enable inherited in-app screen sharing integration. |
| `screenSharing.thumbnail.enabled` | `boolean` | `false` | Automatically show the preview thumbnail window when screen sharing starts. |
...
| `idleDetection.stateFile` | `string` | `"/tmp/outlook-for-linux-idle-state-$USER"` | Path to the idle state file (supports $USER expansion). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `outlookUnreadCountObserver.enabled` | `boolean` | `true` | Enable extracting the Inbox unread count from Outlook Web's folder list. |

### outlookNewMailNotifier

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `outlookNewMailNotifier.enabled` | `boolean` | `true` | Enable native new mail notifications based on Outlook Web's unread mail list rows. |

### media

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `media.microphone.disableAutogain` | `boolean` | `false` | Disable microphone auto gain control so the web app does not automatically adjust microphone volume levels. |
...
| `media.microphone.ignoreSystemMute` | `boolean` | `false` | Stop the web app's mute button from following the operating system microphone mute on Linux. Chromium reports the OS capture-source mute to the page as a track mute event; enable this to keep the web app button where you left it and rely solely on your system/hotkey mute. |
| `media.microphone.overrideConstraints.enabled` | `boolean` | `false` | Enable overriding microphone audio constraints requested via getUserMedia; only the keys you set are overridden. |
...
| `media.camera.resolution.mode` | `string` | `"remove"` | Resolution mode: remove strips web app constraints to allow native camera resolution, override sets a specific width/height. |
...
| `media.camera.autoAdjustAspectRatio.enabled` | `boolean` | `false` | Reapply proper aspect ratio constraints to fix camera video stretching when moving the web app between monitors with different orientations. |
...
| `mqtt.topicPrefix` | `string` | `"outlook"` | Topic prefix for all MQTT messages. |
