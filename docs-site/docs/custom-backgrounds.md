...
:::tip
See the [example README](https://github.com/IsmaelMartinez/outlook-for-linux/tree/main/app/customBackground/example/README.md) for a complete implementation example.
:::
...
# Configure Outlook for Linux
outlook-for-linux --isCustomBackgroundEnabled=true --customBGServiceBaseUrl=http://localhost:8080
```
...
Add to your `~/.config/outlook-for-linux/config.json`:
...
2. **Restart application**: Changes may require restarting Outlook for Linux
...
```bash
outlook-for-linux --logConfig='{"level":"debug"}' --isCustomBackgroundEnabled=true
```
